...
system.initialize = function () {
	...
	// register for "block_interacted_with" event
	...
	this.listenForEvent("minecraft:block_interacted_with", (eventData) => {
		...
		// get block JS object
		let block = this.getBlock(tickingArea, eventData.data.block_position);
		// get block nbt
		let blockstateComponent = this.getComponent(block, "minecraft:blockstate");
		// send infos to chat
		this.tellPlayer("@a", playername + " interacted with '" + block.__identifier__ + "'. NBT Data: " + this.ObjToStr(blockstateComponent.data));
	});
};
